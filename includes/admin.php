...
function rm_enqueue_assets( $hook ) {
	if ( $hook !== 'toplevel_page_' . RM_SLUG ) return;
...
function rm_settings_page() {
	...
	<div class="wrap rm-wrap">
		...
		<div class="rm-header">
			...
			<h1>Numeris Resend <span class="rm-version">v<?php echo esc_html( RM_VERSION ); ?></span></h1>
		</div>
		...
		<nav class="rm-tabs">
			...
			<a href="<?php echo esc_url( add_query_arg( [ 'page' => RM_SLUG, 'tab' => $key ], admin_url( 'admin.php' ) ) ); ?>"
...
function rm_ajax_send_test() {
	...
	$subject = 'Numeris Resend — Test Email';
	$body    = '<h2 style="margin:0 0 16px;font-family:' . esc_attr( rm_opt( 'heading_font', 'Georgia, "Times New Roman", serif' ) ) . ';font-size:24px;font-weight:600;color:' . esc_attr( rm_opt( 'text_color', '#1a1a1a' ) ) . ';">Test email ✓</h2>'
	         ...
	         . '<p style="margin:0;color:#6b7280;font-size:14px;">Sent via Numeris Resend &nbsp;·&nbsp; ' . esc_html( gmdate( 'Y-m-d H:i:s' ) ) . ' UTC</p>';
...
function rm_ajax_preview_email() {
	...
	$body = '<h2 style="margin:0 0 16px;font-family:' . esc_attr( $heading_font ) . ';font-size:24px;font-weight:600;color:' . esc_attr( $text_color ) . ';">Preview email ✓</h2>'
	      ...
	      . '<p style="margin:0;color:#6b7280;font-size:14px;">Generated by Numeris Resend &nbsp;·&nbsp; ' . esc_html( get_bloginfo( 'name' ) ) . '</p>';
